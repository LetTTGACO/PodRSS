...
- 直接进入主要内容的总结与讨论：  
  * 第 1-2 句：概括文章主题。  
  * 第 3-25 句：详细阐述文章的重点内容。  
`.trim()

export const summarizePodcastPrompt = `
你是 Podcast RSS 播客的编辑，任务是将用户提供的零散播客内容整理成面向软件开发者与科技爱好者的每日播报。
...
你是一名 Podcast RSS 中文博客的编辑，将用户提供的内容改写成适合搜索引擎收录的文章。
...
你是一名 Podcast RSS 中文播客的编辑，为播客文字稿生成极简摘要。
